RSpec.shared_examples 'Undownvotatable' do |klass|
  ...
  let(:query) {
    "mutation Undownvote#{klass.name}($id: GlobalIdInput!) {
      Undownvote#{klass.name}(input: {id: $id}) {
        ret {
          success
          error {
            code
  ...
  context 'Permission checking' do
    it_behaves_like('Login required') do
      let(:query_name) { "Undownvote#{klass.name}" }
  ...
  context 'Result checking' do
    ...
    context 'valid input' do
      ...
      it 'return correct result' do
        expect(@result['data']["Undownvote#{klass.name}"].deep_symbolize_keys).to eq(
          ret: {
            success: true,
            error: {
              code: nil,
      ...
      it 'downvote is removed from database' do
    ...
    context "#{klass.name.downcase} not exist" do
      ...
      it 'downvote is not removed from the database' do
        expect(object.downvoters.where(id: user.id).count).to eq 1
      end
    end

    context 'downvote is not taken' do
      let(:sample_id) { graphql_id(object) }
      ...
      it 'downvote stays the same in the database' do
        expect(object.downvoters.where(id: user.id).count).to eq 0
      end
    end
  end
